...
test("SQL adapter binds values safely, maps rows and keeps statements immutable", async () => {
  ...
  try {
    ...
    await db
      .prepare(
        "CREATE TABLE sample (id INTEGER PRIMARY KEY, value TEXT, reading REAL)",
      )
      .run();
    ...
    assert.deepEqual(
      await db.prepare("SELECT * FROM sample WHERE id=?").bind(1).first(),
      {
        id: 1,
        value: malicious,
        reading: 16.2,
    ...
    assert.equal(
      (await db.prepare("SELECT * FROM sample").all()).results.length,
      2,
    );
    assert.equal(
      await db.prepare("SELECT * FROM sample WHERE id=99").first(),
      null,
    );
    assert.equal(
      (await db.prepare("SELECT * FROM sample WHERE id=2").first()).value,
      null,
    );
...
test("write batches return rows and roll back completely on a constraint failure", async () => {
  ...
  try {
    ...
    const result = await db.batch([
      db.prepare("INSERT INTO sample VALUES (1) RETURNING id"),
    ]);
    ...
    assert.equal(
      await db.prepare("SELECT * FROM sample WHERE id=2").first(),
      null,
    );
...
test("existing schema works with libSQL, including foreign keys and chat cascade", async () => {
  ...
  try {
    ...
    await db
      .prepare(
        "INSERT INTO staff VALUES ('staff','Name','owner@example.test','hash','admin',1,1)",
      )
      .run();
    await db
      .prepare(
        "INSERT INTO sessions (token_hash,kind,staff_id,expires_at,created_at) VALUES ('session','staff','staff',2,1)",
      )
      .run();
    await db
      .prepare(
        "INSERT INTO chat_messages VALUES ('message','session','user','Hello',1)",
      )
      .run();
    await db.prepare("DELETE FROM sessions WHERE token_hash='session'").run();
    assert.equal(
      (await db.prepare("SELECT * FROM chat_messages").all()).results.length,
      0,
    );
    await assert.rejects(() =>
      db
        .prepare(
          "INSERT INTO chat_messages VALUES ('orphan','missing','user','Hello',1)",
        )
        .run(),
    );
  } finally {
