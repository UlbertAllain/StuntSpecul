...
async function start(mode, variables) {
  server = spawn(process.execPath, ["node_modules/next/dist/bin/next", mode, "--port", "3100"], {
    env: variables,
    stdio: "ignore",
  });
  let startupError;
  ...
  while (Date.now() < deadline) {
    ...
    try {
      await fetch(origin + "/api/config", { signal: AbortSignal.timeout(1000) });
      return;
    } catch {
      await delay(250);
    }
...
try {
  ...
  for (const asset of ["mimo-cheer.png", "mimo-stand.png", "stuntspecula-logo.jpeg"]) {
    const response = await fetch(origin + "/images/" + asset);
    assert.equal(response.status, 200);
  }
  await stop();

  execFileSync(process.execPath, ["scripts/migrate.mjs"], { env: environment, stdio: "inherit" });
  ...
  assert.equal((await data(await request("/auth/me", "GET", undefined, staffCookie))).role, "admin");
  assert.equal((await request("/children")).status, 401);
  assert.equal((await request("/children", "POST", {}, staffCookie, "https://evil.test")).status, 403);
  const birth = new Date();
  birth.setUTCFullYear(birth.getUTCFullYear() - 3);
  const child = await data(await request("/children", "POST", {
    code: "HTTP001", name: "Anak pengujian", birthDate: birth.toISOString().slice(0, 10),
    sex: "male", guardian: "Orang tua pengujian",
  }, staffCookie), 201);
  const exam = await data(await request("/examinations", "POST", {
    childId: child.id, canStand: true, cameraEnabled: false,
  }, staffCookie), 201);
  await data(await request(`/mirror/examinations/${exam.id}/claim`, "POST", {}, staffCookie));
  await data(await request(`/mirror/examinations/${exam.id}/complete`, "POST", {
    heightCm: null, weightKg: null, captureStatus: "skipped",
  }, staffCookie));
  await data(await request("/examinations", "GET", undefined, staffCookie));
  const link = await data(await request(`/examinations/${exam.id}/access`, "POST", {}, staffCookie));
  const exchanged = await request("/parent/exchange", "POST", { code: new URL(link.url).hash.slice(1) });
  await data(exchanged.clone());
  const parentCookie = exchanged.headers.get("set-cookie").split(";")[0];
  const result = await data(await request("/parent/result", "GET", undefined, parentCookie));
  assert.equal(result.result.id, exam.id);
  assert.equal(result.result.growthStatus, "unavailable");
  const chat = await request("/parent/chat", "POST", { message: "Apa arti hasilnya?", consent: true }, parentCookie);
  assert.equal(chat.status, 503);
  assert.equal((await chat.json()).code, "ai_not_configured");
  console.log("HTTP smoke passed: production API/assets, setup, staff, examination, history, parent QR, AI configuration.");
} finally {
  await stop();
